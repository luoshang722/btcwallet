...
func (s *Store) Spender(dbtx walletdb.ReadTx, out *wire.OutPoint) (*wire.MsgTx, uint32, error) {
	...
	if v != nil {
		...
		if v == nil {
			return nil, 0, apperrors.E{ErrorCode: apperrors.ErrInput, Description: "output is not a credit"}
		}
		...
		if v == nil {
			return nil, 0, apperrors.E{ErrorCode: apperrors.ErrNoExist, Description: "credit is unspent"}
		}
	...
	if v == nil {
		return nil, 0, apperrors.E{ErrorCode: apperrors.ErrData, Description: "missing unmined spending tx"}
	}
	err := spender.Deserialize(bytes.NewReader(extractRawUnminedTx(v)))
	if err != nil {
		return nil, 0, apperrors.E{ErrorCode: apperrors.ErrData, Err: err}
	}
	found := false
	...
	if !found {
		return nil, 0, apperrors.E{ErrorCode: apperrors.ErrData, Description: "recorded spending tx does not spend credit"}
	}
	return &spender, spenderIndex, nil
}
